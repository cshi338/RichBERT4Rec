...
def map_column(df: pd.DataFrame, col_name: str):
    ...
    #CHANGE THIS IF NOT USING SEP
    mapping = {k: i + 3 for i, k in enumerate(values)}
...
def get_context(df: pd.DataFrame, split: str, context_size: int = 120, val_context_size: int = 1):
    ...
    """
    if split == "train":
        end_index = random.randint(10, df.shape[0] - 2)
    elif split in ["val"]:
...
def pad_list(list_integers, history_size: int, pad_val: int = PAD, mode="left"):
    ...
    if len(list_integers) < history_size:
        if mode == "left":
            list_integers = np.concatenate([np.array([pad_val] * (history_size - len(list_integers))),list_integers])
        else:
            list_integers = np.concatenate([list_integers,np.array([pad_val] * (history_size - len(list_integers)))])
    return list_integers
    """
